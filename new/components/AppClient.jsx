...
/* ═══════════════════════════════════════════════════
   Utility — send image + prefs to Next.js API route
   ═══════════════════════════════════════════════════ */
async function fetchRecipes(base64Images, preferences) {
  ...
  if (!res.ok) {
    if (res.status === 429 || res.status === 503) {
      throw new Error("Gemini API rate limit reached. Please wait a minute and try again, or use a new API key.");
    }
    const detail = await res.json().catch(() => null);
    const msg = detail?.error || `Something went wrong (${res.status})`;
    throw new Error(msg);
  }
  return res.json();
}
...
function HomeScreen({
  ...
  return (
    <div className="flex flex-col min-h-dvh px-6 py-8 bg-pantry pt-safe">
      ...
      {isGuest && (
        <p className="stagger-6 mt-auto pt-8 text-center text-xs pb-safe" style={{ color: "var(--sage-400)" }}>
...
function RecipeCard({ recipe, isFav, onToggleFav }) {
  ...
  return (
    <div className="min-w-[280px] max-w-[300px] flex-shrink-0 rounded-2xl p-5 card-elevated flex flex-col gap-3">
...
function ResultsScreen({ recipes, detectedIngredients, scanError, favoriteIds, onToggleFav, onReset, onHome, isGuest }) {
  return (
    <div className="min-h-dvh px-6 py-10 flex flex-col gap-6 bg-pantry animate-fade-in-up pt-safe">
      ...
      <div className="mt-auto w-full max-w-xs mx-auto flex flex-col gap-3 stagger-4 pb-safe">
        <button
          onClick={onReset}
    ...
    const unsub = onAuthStateChanged(auth, (fbUser) => {
      if (fbUser) {
        ...
        Promise.race([
          ...
          new Promise((_, rej) =>
            setTimeout(() => rej(new Error("Firestore timeout")), 10000)
          ),
        ])
          .then((snap) => {
            if (snap.exists()) {
